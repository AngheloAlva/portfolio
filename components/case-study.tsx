...
function hasHeadline(block: ParagraphBlock): block is { headline: string; body: string } {
	return typeof block !== "string"
}

function ParagraphBlocks({ blocks }: { blocks: ReadonlyArray<ParagraphBlock> }): ReactNode {
	return (
	...
	return (
		<main id="main-content" className="flex-1">
			...
			<section className="border-border relative border-b">
				...
				<div className={`border-border grid grid-cols-2 border-t ${metaColsClass}`}>
					{meta.map((item) => (
						<div
							key={item.label}
						...
						>
							...
							<p className="text-foreground mt-3 text-sm leading-snug">{item.value}</p>
						</div>
					))}
				</div>
				...
				{cs.visualPrivacy === "confidential-ui" ? (
					<div className="border-border bg-muted border-t px-6 py-5 sm:px-10 lg:px-14">
						<p className="text-muted-foreground max-w-3xl text-xs leading-relaxed sm:text-sm">
							Mockup ilustrativo. El sistema real es privado por acuerdo con el cliente — sin
							capturas ni datos reales.
						</p>
					</div>
				) : null}
				<SectionCorners />
						...
						<div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
							{cs.metrics.map((metric) => (
								<div key={metric.label}>
									...
									<p className="text-foreground mt-3 text-sm font-medium">{metric.label}</p>
									{metric.caption ? (
                  ...
                  the remaining cards fill the row below without leaving a gap. */}
							{(() => {
								...
								return cs.features.map((feature, i) => {
									...
									const glyphClass = isLead ? "flex-1 min-h-40 sm:min-h-52" : "flex-1 min-h-24"
									return (
										<article
											key={feature.title}
