          ...
          {i < steps - 1 ? (
            <span
              className={`h-px flex-1 ${
                i < done - 1 ? "bg-foreground/40" : "bg-muted"
              }`}
            />
          ) : null}
        </Fragment>
    ...
    <WFGlyphCard className={className}>
      ...
      <div className="mt-2.5 flex gap-1.5">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="border-border flex flex-1 flex-col gap-1 rounded-md border p-1.5"
          >
  ...
  return (
    <WFGlyphCard className={className}>
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex gap-1">
          {Array.from({ length: 3 }).map((_, i) => (
            <span
              key={i}
              className="border-border h-3 w-7 rounded-full border"
            />
          ))}
        </div>
      </div>
      <div className="mt-2 flex flex-1 flex-col justify-center gap-1.5">
        {/* parent + sub rows */}
        {[{ indent: false }, { indent: true }, { indent: true }].map(
          (row, i) => (
            <div key={i} className="flex items-center gap-2">
              <span
                className={`h-3 ${row.indent ? "ml-3 w-10" : "w-12"} bg-muted rounded`}
              />
...
function CapacityGlyph({ className }: { className?: string }): ReactNode {
  ...
  const heat = [
    0.15, 0.4, 0.7, 0.25, 0.9, 0.55, 0.1, 0.6, 0.3, 0.8, 0.45, 0.2, 0.95, 0.5,
    0.35, 0.75, 0.15, 0.65, 0.4, 0.85, 0.3,
  ];
  return (
    <WFGlyphCard className={className}>
      <WFBar w={55} h={20} tone="mid" />
      ...
      <div className="border-border bg-background/60 flex h-full flex-col rounded-md border p-2.5">
        ...
        <div className="border-border bg-background mt-1.5 flex items-end justify-center gap-0.5 rounded border p-1.5">
          {bars.map((w, i) => (
            <span
              key={i}
              className="bg-foreground/75 h-5"
              style={{ width: w }}
            />
          ))}
        </div>
 ...
 */
export const busancFeatureGlyphs: Record<
  string,
